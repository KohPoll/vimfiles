...
" Modified by Kohpoll 
"   - adjust the structure;remove some tags;add html5,css3;change the color of braces
...
syn keyword cssTagName caption col colgroup table thead th tbody tfoot tr td
syn keyword cssTagName form fieldset legend input label optgroup option select button textarea
syn keyword cssTagName area img map object param iframe frameset frame noscript noframes
...
syn region cssFunction contained matchgroup=cssFunctionName start="\<\(rgb\|rgba\|clip\|attr\|counter\|rect\)\s*(" end=")" oneline keepend

syn match cssFilterProp contained "\(-ms-\|\)filter\>"
...
" css3 gradient
syn match cssGradientAttr contained "\(-\(moz\|ms\|o\|webkit\)-\|\)linear-gradient\>"
syn match cssGradientAttr contained "-webkit-gradient\>"
...
syn match cssTextProp contained "\<text-indent\>"
...
"syn match cssTextAttr contained "\(-\(moz\)-\|\)\(\(pre\)-\|\)wrap\>"
syn match cssTextProp contained "\<word-wrap\>"
syn match cssTextProp contained "\<text-wrap\>"
...
syn match cssBoxProp contained "\(-\(moz\|webkit\|ms\|o\)-\|\)border\(-\(bottom-left\|bottom-right\|top-right\|top-left\)\)-radius\>"
